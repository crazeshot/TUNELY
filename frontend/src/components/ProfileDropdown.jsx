...
export default function ProfileDropdown() {
  const { user, isLoggedIn, logout, openLogin, openRegister, setIsWrappedOpen } = useAuth();
  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      ...
      {isLoggedIn ? (
        ...
        <button
          onClick={() => setIsOpen((prev) => !prev)}
          className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/15 hover:border-white/30 transition-all shadow-md group text-xs font-semibold text-white/90 hover:text-white"
          title="Account Menu"
        >
          <div className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center text-white/70 group-hover:text-white transition-colors">
            <User size={13} />
          </div>
          <span>Account</span>
          <ChevronDown
      ...
      {isOpen && (
        <div
          className={`absolute right-0 top-full mt-2 rounded-3xl border border-white/15 shadow-[0_20px_50px_rgba(0,0,0,0.85)] text-white z-50 animate-fade-in ${
            isLoggedIn ? 'w-72 p-2' : 'w-56 p-2.5'
          }`}
          style={{
        ...
        >
          {isLoggedIn ? (
            ...
            /* Guest / Logged Out: Show ONLY Login and Sign Up buttons */
            <div className="flex flex-col gap-2 p-1">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl text-xs font-bold bg-white hover:bg-neutral-200 text-black shadow-[0_0_15px_rgba(255,255,255,0.25)] transition-all hover:scale-102 active:scale-98"
              >
                <LogIn size={14} />
                <span>Log In</span>
              </button>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl text-xs font-bold bg-white/10 hover:bg-white/15 text-white border border-white/15 transition-all hover:scale-102 active:scale-98"
              >
                <UserPlus size={14} />
                <span>Sign Up</span>
              </button>
        ...
        >
          ...
          >
            <img
              src="/favicon.svg"
              alt="Tunely"
              className="h-12 w-auto object-contain mx-auto shadow-[0_0_20px_rgba(255,255,255,0.3)]"
            />
