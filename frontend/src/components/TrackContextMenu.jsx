  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-pink-200 font-medium">Start Track Radio</span>
      </button>
      ...
      >
        ...
        >
          <span className="flex items-center gap-2.5">
            <PlusCircle size={13} className="text-white/70" />
        ...
        {showPlaylists && (
          <div
            className="absolute left-full top-0 ml-1 min-w-44 py-1.5 rounded-2xl shadow-2xl backdrop-blur-2xl border border-white/15"
            style={{ background: 'rgba(25, 12, 35, 0.98)' }}
          >
            {playlists.length === 0 ? (
              <p className="px-3.5 py-2 text-white/40 text-[11px]">No playlists</p>
            ) : (
              playlists.map((pl) => (
                <button
                  key={pl.id}
                  className="w-full px-3.5 py-1.5 hover:bg-white/10 text-left truncate text-white/80 hover:text-white transition-colors"
                  onClick={() => {
      ...
      >
        <Heart size={13} className={isLiked ? 'text-pink-400 fill-pink-400' : 'text-white/70'} />
