...
function initLiquidEtherEngine(container, props, { isVisibleRef, rafRef }) {
  const {
    mouseForce = 20,
    cursorSize = 10,
    isViscous = false,
  ...
  class CommonClass {
    ...
    update() {
      ...
      this.delta = Math.min(Math.max(rawDelta, 0.008), 0.024);
      this.lastTime = now;
  ...
  class MouseClass {
    ...
    init(container) {
      ...
      this.docTarget = document.documentElement;
      const defaultView =
    ...
    isPointInside(clientX, clientY) {
      if (!this.rect || this.rect.width === 0 || this.rect.height === 0) return false;
      return clientX >= this.rect.left && clientX <= this.rect.right && clientY >= this.rect.top && clientY <= this.rect.bottom;
    }
    ...
    setCoords(x, y) {
      ...
      const nx = (x - this.rect.left) / this.rect.width;
      const ny = (y - this.rect.top) / this.rect.height;
    ...
    onDocumentMouseMove(event) {
      ...
      if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
        if (!this.rect || this.rect.width === 0 || this.rect.height === 0) return;
        const nx = (event.clientX - this.rect.left) / this.rect.width;
        const ny = (event.clientY - this.rect.top) / this.rect.height;
    ...
    update() {
      ...
      // Clamp diff magnitude to avoid teleport spikes and shuttering
      const diffLen = this.diff.length();
      if (diffLen > 0.12) {
        this.diff.multiplyScalar(0.12 / diffLen);
      }
      this.coords_old.copy(this.coords);
...
`;
  const color_frag = `
    ...
    vec3 c = texture2D(palette, vec2(pow(lenv, 0.75), 0.5)).rgb;
    float alpha = pow(smoothstep(0.02, 0.9, lenv), 1.4) * 0.45;
...
`;
  const externalForce_frag = `
    ...
    d = d * d * (3.0 - 2.0 * d); // Smooth hermite cubic spline falloff for ultra-smooth fluid edges
...
function LiquidEther({
  ...
  }, []); // Mount WebGL engine once to prevent frame-destroy blinking
